...
description: >-
  ...
  in any app. Use whenever work changes what a user sees, understands,
  chooses, or does: shaping requirements and flows; building or redesigning
  pages and components; reviewing URLs, screenshots, diffs, or agent
  findings; improving product copy, information architecture, component choice,
  design-system compliance, hierarchy, layout, interaction, accessibility, responsive
...
Make the interface correct for the user, the product, and the team. Working code is not enough: choose the right interaction, make scope and consequences clear, cover reality beyond the happy path, and verify the rendered result.
...
3. Repository-canonical guidance: `AGENTS.md`, the project's design-system component APIs and style guide, and routed skills.
...
Before proposing UI, read the applicable `AGENTS.md`/`CLAUDE.md` chain, supplied briefs and designs, and the product logic that determines mutations, permissions, validation, errors, and side effects.
...
Inventory entry points, visible regions, overlays, transitions, exits, and return paths. Map only reachable states including loading, empty, sparse, populated, validation, error, permission, disabled, optimistic, stale, destructive, and responsive variants. See `resilience.md`.

### 5. Load the routed references

| Need | Load |
| ---- | ---- |
| Product/flow/component decision | `product-judgment.md` + `Skill(impeccable)` |
| Implementation, material visual change, or full review | `interface-quality.md` |
| Copy or accessible names | `copy.md` + `surfaces.md` |
| Layout, typography, color, spacing, component APIs | `Skill(taste-skill)` + the project's design system |
| Keyboard, focus, forms, touch, animation, URL state, performance | `Skill(emil-design-eng)` + `resilience.md` |
| Overflow, localization, extreme data, network/error resilience | `resilience.md` |
| Motion/animation review of visible changes | `Skill(review-animations)` |
...
7. Load `Skill(review-animations)` for structural visible motion changes.
